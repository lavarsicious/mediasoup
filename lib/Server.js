...
class Server extends EnhancedEventEmitter
{
	 ...
	 * @param {Number} [workerIdx] - Specific worker to use (from 0 to numWorkers - 1).
	 *
	 * @return {Room}
	 */
	Room(mediaCodecs, { workerIdx } = {})
	{
		logger.debug('Room()');
	...
	_addWorker(worker)
	{
		...
		worker.on('@close', () =>
		{
			...
			// Force last used worker to be 0 to avoid having it pointing to
			// a idx that no longer exists after this worker closure.
			this._latestWorkerIdx = 0;
