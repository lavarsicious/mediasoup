...
class Peer extends EnhancedEventEmitter
{
	 ...
	 * @return {Consumer}
	 */
	getConsumerForSource(source)
	{
		return this.consumers
			.find((consumer) => consumer.source === source);
	}
			...
			.then(() =>
			{
				switch (method)
				{
					case 'createTransport':
					{
						...
						{
							plainRtpParameters.preferIPv4 = Boolean(options.preferIPv4);
							plainRtpParameters.preferIPv6 = Boolean(options.preferIPv6);

							return this._createPlainRtpTransport(
								id, direction, plainRtpParameters, appData)
								.then((transport) =>
								{
									const response =
									{
		...
		return Promise.resolve()
			.then(() =>
			{
				switch (method)
				{
					...
					{
						const { id, dtlsParameters, plainRtpParameters } = notification;
						const transport = this._transports.get(id);
						...
						else if (plainRtpParameters)
						{
							return transport.setRemoteParameters(plainRtpParameters)
								.catch((error) =>
								{
									transport.close();
					...
					{
						const { id, interval } = notification;
						const transport = this._transports.get(id);
	...
	addConsumerForProducer(producer, notifyClient = true)
	{
		...
		this._channel.request(
			'router.createConsumer', internal, { kind: consumer.kind })
			.then(() =>
			{
	...
	_createPlainRtpTransport(id, direction, options, appData)
	{
		logger.debug('_createPlainRtpTransport() [id:%s, direction:%s]', id, direction);
		...
		return this._channel.request(
			...
			.then((data) =>
			{
				...
				// Store the PlainRtpTransport and remove it when closed.
				this._transports.set(transport.id, transport);
				transport.on('@close', () => this._transports.delete(transport.id));
	...
	_createProducer(id, kind, transportId, rtpParameters, remotelyPaused, appData)
	{
		...
		else if (this._data.spy)
			return Promise.reject(new Error('spy peers cannot create producers'));
